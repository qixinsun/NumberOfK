...
void NumberOfK(int* data, int length, int k, int start, int end)
{
	if(start > end)
		return;
	...
	if(middleData == k)
	{
		if((k != length - 1 && data[middleIndex+1] != k) || middleIndex == length - 1)
		{
			lastK = middleIndex;
			kExist = true;
		}
        ...
        {
            NumberOfK(data, length, k, middleIndex + 1, end);
        }
		if((k != 0 && data[middleIndex-1] != k) || middleIndex == 0)
		{
			firstK = middleIndex;
			kExist = true;
		}
        ...
        {
            NumberOfK(data, length, k, start, middleIndex - 1);
        }
	}
	else
	{
...
void Test(const char* testName, int data[], int length, int k, int expected)
{
    ...
    lastK = 0;
    kExist = false;
    NumberOfK(data, length, k,  0, length - 1);
    if(testName != nullptr)
        printf("%s begins: ", testName);

    //int result = GetNumberOfK(data, length, k);
    int result = lastK - firstK + 1;
    if(result == expected || !kExist)
        printf("Passed.\n");
    else
        printf("Failed.\n");
}
